...
void setup() {
  ...
  calibrateIMUs();
  Serial.println("IMUs calibrated.");
...
void calibrateIMUs() {
  ...
  for (int i = 0; i < num_samples; i++) {
    ...
    for (int j = 0; j < 3; j++) {
      accel_offsets[j] += (accel1[j] + accel2[j]) / 2.0;
    }
    delay(10);
  }

  for (int i = 0; i < 3; i++) {
    accel_offsets[i] /= num_samples;
  }
}
...
void loop() {
  ...
  if (current_time - last_time >= dt * 1000) {
    ...
    // Average the two IMU data sets and apply offsets
    float avg_accel[3];
    for (int i = 0; i < 3; i++) {
      avg_accel[i] = ((accel1[i] + accel2[i]) / 2.0) - accel_offsets[i];
    }
    ...
    Serial.print(velocity[0]); Serial.print(", ");
...
void updateDistances() {
  for (int i = 0; i < 6; i++) {
    ...
    switch (i) {
      case 0: distances[0] = measureDistance(ECHO_PIN_1); break;
      case 1: distances[1] = measureDistance(ECHO_PIN_2); break;
      case 2: distances[2] = measureDistance(ECHO_PIN_3); break;
      case 3: distances[3] = measureDistance(ECHO_PIN_4); break;
      case 4: distances[4] = measureDistance(ECHO_PIN_5); break;
      case 5: distances[5] = measureDistance(ECHO_PIN_6); break;
    }
    delay(50); // Ensure sensors do not interfere with each other
  }
}
